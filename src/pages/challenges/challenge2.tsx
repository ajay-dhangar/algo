...
const DataStructuresQuiz = () => {
  const questions = [
    {
      question: "1. Let A[1...n] be an array of n distinct numbers. If i < j and A[i] > A[j], then the pair (i, j) is called an inversion of A. What is the expected number of inversions in any permutation on n elements?",
      options: [
        "n(n-1)/2",
        "n(n-1)/4",
        "n(n+1)/4",
    ...
    {
      question: "2. Consider a two-dimensional array A[20][10]. Assume 4 words per memory cell, the base address of array A is 100, elements are stored in row-major order and first element is A[0][0]. What is the address of A[11][5]?",
      options: ["560", "460", "570", "575"],
      answer: "570",
    },
    {
      question: "3. An array A consists of n integers in locations A[0], A[1] ....A[n-1]. It is required to shift the elements of the array cyclically to the left by k places, where 1 <= k <= (n-1). Which is the correct algorithm to complete this?",
      options: [
    ...
    {
      question: "5. A three-dimensional array in C++ is declared as int A[x][y][z]. The address of an item at the location A[p][q][r] can be computed as:",
      options: [
    ...
    {
      question: "6. Let A be a square matrix of size n x n. Consider the following program. What is the expected output?",
      options: [
        "The matrix A itself",
    ...
    {
      question: "7. Assume that the size of an integer is 32 bit. What is the output of the following program?",
    ...
    },
    {
      ...
      answer: "Garbage character followed by 'G', followed by more garbage characters",
    },
    {
      question: "12. Which of the following ways can be used to declare a node for a singly linked list?",
      options: ["TRUE", "FALSE"],
      answer: "TRUE",
    },
    {
    ...
    {
      question: "15. The time complexity of calculating the sum of all leaf nodes in an n-order binary tree is __________",
      options: ["O(n^2)", "O(n+1)", "O(1)", "O(n)"],
      answer: "O(n)",
    },
    {
      question: "16. An important application of a binary tree is ______",
      options: ["Huffman coding", "Stack implementation", "Queue implementation", "Traverse a cyclic graph"],
      answer: "Huffman coding",
    },
    {
      question: "17. In a full binary tree, if there are L leaves, then the total number of nodes N are?",
      options: ["N = 2*L", "N = L + 1", "N = L – 1", "N = 2*L – 1"],
      answer: "N = 2*L – 1",
    },
    {
      question: "18. Which among the following is the pseudo code for post-order traversal?",
      options: [
      ...
      answer: "Order(node): if node is not null: Order(node.left) Order(node.right) print node.value",
    },
    {
      question: "19. For the expression (7-(4*5))+(9/3), which of the following is the post order tree traversal?",
      options: ["*745-93/+", "93/+745*-", "745*-93/+", "74*+593/-"],
      answer: "*745-93/+",
    },
    {
      question: "20. Which traversal is shown by the pseudo code?\nOrder(node):\n  if node is not null:\n   Order(node.left)\n  print node.value\n    Order(node.right)",
      options: ["In-order", "Level Order", "Post-order", "Pre-order"],
      answer: "In-order",
    },
    // Questions 21 to 30
    {
      question: "21. Which of the following ways can be used to represent a graph?",
      options: [
  ...
  return (
    <Layout title="Data Structures Quiz" description="Test your knowledge of data structures with this quiz.">
      <div style={{ padding: "20px" }}>
        <h1>Data Structures Challenge-2</h1>
        {showResult ? (
          <div>
