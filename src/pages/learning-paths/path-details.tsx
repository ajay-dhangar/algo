...
import { getLearningPathById, LearningPath } from "../../data/learningPaths";
...
const PathDetailsContent: React.FC<PathDetailsContentProps> = ({ path }) => {
  ...
  useEffect(() => {
    const progress = safeJsonParse<{ [key: string]: boolean }>("algo_progress", {});
    ...
    const inProgress = safeJsonParse<{ [key: string]: boolean }>("algo_in_progress", {});
  ...
  // Synchronize telemetry event listener
  useEffect(() => {
    const handleProgressUpdate = (e: Event) => {
      const customEvent = e as CustomEvent<{ topicId: string; completed: boolean }>;
      const { topicId, completed } = customEvent.detail;
  ...
  const stats = useMemo(() => {
    ...
    const totalHours = path.totalHours;
  ...
  const toggleCompleted = (topicId: string) => {
    const nextState = !completedTopics.has(topicId);
    const newCompletedSet = new Set(completedTopics);
    
    if (nextState) {
      newCompletedSet.add(topicId);
      setInProgressTopics((prev) => {
        const updated = new Set(prev);
        updated.delete(topicId);
        
        // Sync in progress state to local storage
        const inProgStore = safeJsonParse<{ [key: string]: boolean }>("algo_in_progress", {});
        delete inProgStore[topicId];
        localStorage.setItem("algo_in_progress", JSON.stringify(inProgStore));
        
        return updated;
      });
    } else {
      newCompletedSet.delete(topicId);
    }
  ...
  const toggleInProgress = (topicId: string) => {
    ...
    const newInProgressSet = new Set(inProgressTopics);
    
    if (nextState) {
      newInProgressSet.add(topicId);
      setCompletedTopics((prev) => {
        const updated = new Set(prev);
        if (updated.has(topicId)) {
          updated.delete(topicId);
          
          // Sync completed state to local storage
          const progress = safeJsonParse<{ [key: string]: any }>("algo_progress", {});
      ...
      newInProgressSet.delete(topicId);
    }
    setInProgressTopics(newInProgressSet);

    // Sync in progress state to local storage
    const inProgressStore = safeJsonParse<{ [key: string]: boolean }>("algo_in_progress", {});
    inProgressStore[topicId] = nextState;
    localStorage.setItem("algo_in_progress", JSON.stringify(inProgressStore));
  };
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <Link to="/learning-paths" className="inline-flex items-center text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium">
      ...
      <Layout title="Path Not Found">
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <Link to="/learning-paths" className="inline-block bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700">
