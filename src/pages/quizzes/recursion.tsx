...
const RecursionQuiz: React.FC = () => {
  ...
  const fetchAttempts = async (uId: string) => {
    try {
      setApiError(null);
      const res = await axios.get(buildApiUrl(apiBaseUrl, `/api/quiz-attempts/${uId}/recursion`));
      ...
      console.error("Error fetching recursion quiz history:", e);
    }
  };
  ...
  const submitAttempt = async (finalAnswers: string[]) => {
    ...
    try {
      ...
      console.error("Failed to submit recursion quiz attempt:", e);
