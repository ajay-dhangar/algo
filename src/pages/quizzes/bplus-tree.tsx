...
const BPlusTreeQuiz: React.FC = () => {
  ...
  const fetchAttempts = async (uId: string) => {
    try {
      setApiError(null);
      const res = await axios.get(buildApiUrl(apiBaseUrl, `/api/quiz-attempts/${uId}/bplus-tree`));
      ...
      console.error("Error fetching B+ Tree quiz history:", e);
    }
  };
  ...
  const submitAttempt = async (finalAnswers: string[]) => {
    ...
    try {
      ...
      console.error("Failed to submit B+ Tree quiz attempt:", e);
