  ...
  const submitAttempt = async (finalAnswers: string[]) => {
    ...
    try {
      await axios.post(buildApiUrl(apiBaseUrl, "/api/quiz-attempts"), {
        userId,
        quizId: "hash-indexing",
        userAnswers: finalAnswers,
        timeSpent
      });
      fetchAttempts(userId);
    } catch (e) {
      console.error("Failed to submit hash indexing quiz attempt:", e);
    }
  };
    ...
    return (
      <Layout title="Hash Indexing Quiz" description="Test your knowledge of static/dynamic hashing, extendible/linear hashing, and collision handling.">
        <div className="min-h-screen bg-slate-50 dark:bg-[#0b0f19] flex items-center justify-center p-6 font-sans">
          ...
          >
            <div className="w-14 h-14 bg-rose-500/10 text-rose-500 rounded-2xl flex items-center justify-center text-2xl mx-auto mb-5 border border-solid border-rose-500/20">
              <FaHashtag />
            </div>
              ...
              {!showResult ? (
                ...
                >
                  <div className="bg-slate-50 dark:bg-slate-950 border border-solid border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-8 text-center relative overflow-hidden">
                    ...
                    <div className="inline-flex items-baseline gap-1 text-5xl font-mono font-black text-rose-600 dark:text-rose-500">
                      {score}
