...
export default function PublicProfilePage() {
  ...
  const badgeMarkdown = useMemo(() => {
    ...
    return `[![Algo profile](https://img.shields.io/badge/Algo%20profile-${encodeURIComponent(
      username,
    )}-2563eb?style=for-the-badge&logo=codeforces&logoColor=white)](${badgeUrl})`;
  }, [username, origin]);
  ...
  const title = snapshot.isPublic
    ? `${snapshot.displayName}'s Profile`
    : "Private Profile";
  ...
  if (!snapshot.isPublic) {
    return (
      <Layout
        ...
        description="This profile is restricted or private."
      >
        <main
        ...
        >
          ...
          >
            ...
            <p className="mt-3 text-xs sm:text-sm opacity-75 leading-relaxed font-medium">
              The owner has disabled public telemetry data and metrics sharing
              for this workspace profile.
            </p>
            <Link
  ...
  return (
    <Layout
      title={title}
      description={`Public Algo study profile for ${snapshot.displayName}`}
    >
      <main
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    <span className="text-xs font-mono font-semibold opacity-60 flex items-center gap-1">
                      <FiTerminal className="w-3.5 h-3.5 text-[var(--ifm-color-primary)]" />{" "}
                      @{snapshot.username}
                    </span>
                  </div>

                  <h1
                    className="text-2xl sm:text-4xl font-black tracking-tight m-0"
                    style={{ color: "var(--ifm-heading-color)" }}
                  >
                  ...
                  <p className="m-0 text-xs sm:text-sm opacity-75 max-w-xl leading-relaxed font-medium">
                    {snapshot.bio ||
                      "A dedicated developer actively tracking algorithm mastery, problem-solving streaks, and technical metrics."}
                  </p>
                </div>
              </div>
              ...
              {snapshot.allowBadgeEmbed &&
                ...
                !username.startsWith("[") && (
                  <div className="shrink-0 flex items-center gap-2">
                    ...
                    >
                      <FiUserCheck className="text-emerald-500 w-4 h-4" /> Embed
                      Badge <FiExternalLink className="w-3 h-3 opacity-60" />
                    </Link>
                  </div>
                )}
            </div>
            ...
            {snapshot.visibleSections.includes("solved") && (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest opacity-50">
                    Solved Nodes
                  </span>
                ...
                <div>
                  <p
                    className="text-3xl sm:text-4xl font-black m-0"
                    style={{ color: "var(--ifm-heading-color)" }}
                  >
                    {snapshot.solvedCount.toLocaleString()}
                  </p>
                  <p className="text-xs opacity-70 mt-1 m-0 font-medium">
                    Completed learning milestones and algorithmic challenges.
                  </p>
                </div>
              </div>
            )}
            ...
            {snapshot.visibleSections.includes("quiz-mastery") && (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs opacity-70 mt-1 m-0 font-medium">
                    Evaluated assessments completed with high accuracy.
                  </p>
                </div>
              </div>
            )}
          ...
          {snapshot.allowBadgeEmbed && (
            ...
            >
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div>
                  <h2
                    className="text-base sm:text-lg font-black m-0"
                    style={{ color: "var(--ifm-heading-color)" }}
                  >
                    Quick Markdown Snippet
                  </h2>
                  <p className="text-xs opacity-75 mt-1 m-0 font-medium">
                    Embed this live dynamic badge directly in your GitHub
                    profile README.
                  </p>
                </div>
