...
interface Props { challenge: TreeChallenge; }
...
export default function TreeChallengeLayout({ challenge }: Props) {
  ...
  const runTests = useCallback(() => {
    // We show the test cases and expected output as a guide
    // (Full sandboxed execution is run via the Run Code button)
    const results = challenge.testCases.map((tc) => ({
      pass: false,
      msg: `Input: ${tc.input} | Expected: ${tc.expected} | ${tc.description}`,
    }));
