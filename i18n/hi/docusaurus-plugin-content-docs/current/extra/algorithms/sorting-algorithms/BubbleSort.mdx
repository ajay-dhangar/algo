---
id: bubble-sort-algo  
sidebar_position: 1  
title: बबल सॉर्ट  
sidebar_label: बबल सॉर्ट
description: "Detailed guide on Bubble Sort algorithm, implementation, time complexity, and visualization."
tags: [sorting algorithms, bubble sort, dsa, algorithms]
---

import ComplexityCard from '@site/src/components/ComplexityCard';

### Definition:

Bubble sort is a simple comparison-based sorting algorithm that repeatedly steps through the list, compares adjacent elements, and swaps them if they are in the wrong order. This process continues until the list is sorted.

### वीडियो व्याख्या

<LiteYouTubeEmbed
  id="p__ETf2CKY4"
  params="autoplay=1&autohide=1&showinfo=0&rel=0"
  title="Bubble Sort By Abdul Bari"
  poster="maxresdefault"
  lazyLoad={true}
  webp
/>

### Characteristics:

- **Repeated Comparison**:
  - The algorithm compares each pair of adjacent elements and swaps them if they are out of order. It continues until no more swaps are needed.

- **In-Place Sorting**:
  - Bubble sort operates directly on the input array and does not require extra space, making it an in-place sorting algorithm.

- **Stable**:
  - Since the algorithm only swaps elements when necessary, the relative order of elements with equal values remains the same, making bubble sort a stable algorithm.

- **Simple Implementation**:
  - Bubble sort is easy to implement but not very efficient for large datasets. It's mainly used for educational purposes.

### वीडियो व्याख्या

<LiteYouTubeEmbed
  id="p__ETf2CKY4"
  params="autoplay=1&autohide=1&showinfo=0&rel=0"
  title="Bubble Sort By Abdul Bari"
  poster="maxresdefault"
  lazyLoad={true}
  webp
/>

### Complexity:
<ComplexityCard 
  best="O(n)"
  average="O(n²)"
  worst="O(n²)"
  space="O(1)"
/>

### C++ Implementation:

**Iterative Approach**
```cpp
#include <iostream>
using namespace std;

void bubbleSortIterative(int arr[], int size) {
    for (int i = 0; i < size - 1; i++) {
        bool swapped = false; // To check if any swap happened in the current iteration
        for (int j = 0; j < size - i - 1; j++) {
            if (arr[j] > arr[j + 1]) {
                // Swap arr[j] and arr[j + 1]
                int temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
                swapped = true;
            }
        }
        // If no swap occurred, the array is already sorted
        if (!swapped) {
            break;
        }
    }
}

int main() {
    int arr[] = {64, 34, 25, 12, 22, 11, 90};
    int size = sizeof(arr) / sizeof(arr[0]);

    bubbleSortIterative(arr, size);

    cout << "Sorted array: \n";
    for (int i = 0; i < size; i++) {
        cout << arr[i] << " ";
    }
    cout << endl;

    return 0;
}
```

**Recursive Approach**
```cpp
#include <iostream>
using namespace std;

void bubbleSortRecursive(int arr[], int size) {
    // Base case: If the array is of size 1, it's already sorted
    if (size == 1) {
        return;
    }

    // Perform one pass of bubble sort
    for (int i = 0; i < size - 1; i++) {
        if (arr[i] > arr[i + 1]) {
            // Swap arr[i] and arr[i + 1]
            int temp = arr[i];
            arr[i] = arr[i + 1];
            arr[i + 1] = temp;
        }
    }

    // Recursively sort the remaining array
    bubbleSortRecursive(arr, size - 1);
}

int main() {
    int arr[] = {64, 34, 25, 12, 22, 11, 90};
    int size = sizeof(arr) / sizeof(arr[0]);

    bubbleSortRecursive(arr, size);

    cout << "Sorted array: \n";
    for (int i = 0; i < size; i++) {
        cout << arr[i] << " ";
    }
    cout << endl;

    return 0;
}
```

### Summary:

Bubble sort is one of the simplest sorting algorithms. While it is inefficient for large datasets due to its O(n²) time complexity, it provides an easy-to-understand introduction to sorting. Both the iterative and recursive versions of bubble sort are straightforward to implement, with the iterative version being more commonly used due to its simplicity.

## Algorithm Tip
Bubble Sort is best for learning basic sorting concepts and works well for very small datasets.

## When to Use This Algorithm

Use Bubble Sort when:
- The dataset is very small
- Simplicity is more important than efficiency
- You are learning sorting concepts

## Complexity Comparison Table

| Algorithm | Best Case | Average Case | Worst Case | Space Complexity |
|---|---|---|---|---|
| Bubble Sort | O(n) | O(n²) | O(n²) | O(1) |
| Merge Sort | O(n log n) | O(n log n) | O(n log n) | O(n) |
| Quick Sort | O(n log n) | O(n log n) | O(n²) | O(log n) |

## सामान्य गलतियाँ

- Using Bubble Sort for large datasets where performance becomes very slow
- Forgetting to stop early when the array is already sorted
- Incorrect swapping of adjacent elements

## Common Interview Questions

- **Sort Colors (LeetCode 75)**: Sort an array with 0s, 1s, and 2s in-place (often used to discuss basic sorting stability and swap operations). [Problem Link](https://leetcode.com/problems/sort-colors/)
- **Check if Array is Sorted**: Write a function that returns true if the given array is sorted (uses the early-exit optimization concept of Bubble Sort).
- **Count Swaps in Bubble Sort**: Calculate the exact number of swaps required to sort an array.


## संबंधित विषय

- **[Selection Sort](./SelectionSort.md)**: Simple in-place comparison sorting algorithm ($O(N^2)$).
- **[Insertion Sort](./InsertionSort.md)**: Efficient $O(N^2)$ sort for small or partially sorted arrays.
- **[Quick Sort](./QuickSort.md)**: Fast divide-and-conquer sorting algorithm ($O(N \log N)$ average).