...
description: "Merge Sort is a divide-and-conquer sorting algorithm that splits the array into smaller subarrays, sorts them, and merges them back. Learn with interactive visualization, characteristics, complexity analysis, and C++ implementation."
...
Below is an interactive Merge Sort visualizer. You can adjust the array size, speed, and click **Start Simulation** to watch the divide-and-conquer merging process step-by-step!
...
- **Merge Sorted Array (LeetCode 88)**: Merge two sorted arrays into a single sorted array. [Problem Link](https://leetcode.com/problems/merge-sorted-array/)
- **Sort List (LeetCode 148)**: Sort a linked list in $O(n \log n)$ time using constant auxiliary space (Merge Sort is ideal for lists). [Problem Link](https://leetcode.com/problems/sort-list/)
- **Count Inversions in an Array**: Count the number of inversions (pairs $(i, j)$ such that $i < j$ and $arr[i] > arr[j]$) using the Merge Sort splitting and merging process.
- **Reverse Pairs (LeetCode 493)**: Count the number of reverse pairs in an array using the divide-and-conquer strategy of Merge Sort. [Problem Link](https://leetcode.com/problems/reverse-pairs/)


## संबंधित विषय

- **[Quick Sort](./QuickSort.md)**: Faster average-case divide-and-conquer sort ($O(N \log N)$ avg).
- **[Insertion Sort](./InsertionSort.md)**: Used by TimSort for small sub-arrays ($O(N^2)$ worst case).
- **[Two Pointers & Sliding Window](../two-pointers-sliding-window.md)**: Merge step uses a two-pointer merge of sorted halves.