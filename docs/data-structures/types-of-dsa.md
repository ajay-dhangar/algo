...
Data Structures and Algorithms (DSA) can be broadly classified into different types based on their usage and problem-solving capabilities. This guide will introduce you to the most commonly used data structures and algorithms in computer science, including essential techniques like Sliding Window, KMP, DSU, and more. <AdsComponent />
...
* **Suffix Trees**: Useful for string matching problems. <AdsComponent />
...
Searching algorithms help locate specific elements in a data structure. Additional techniques are crucial for efficient problem-solving.
...
* **Binary Search Variations**: Includes Lower Bound, Upper Bound, and Binary Search on Answer for optimization problems.

* **Sliding Window Technique**: Maintains a dynamic window over an array to solve subarray/substring problems efficiently (e.g., maximum sum subarray of size k).

* **Two Pointers Technique**: Uses two pointers to traverse the array or linked list, often for finding pairs or subarrays meeting certain conditions.

* **Prefix Sum**: Precomputes cumulative sums to answer range sum queries in constant time.
...
* **Topological Sort**: Orders vertices in a directed acyclic graph (DAG) such that for every directed edge uv, vertex u comes before v. Can be implemented using Kahn's Algorithm (BFS-based) or DFS-based approach.

* **Union-Find / Disjoint Set Union (DSU)**: Efficient data structure for managing partitions of a set, supporting union and find operations. Widely used in Kruskal's MST and other connectivity problems.

* **Prim's Algorithm**: Finds the Minimum Spanning Tree (MST) for a weighted undirected graph using a greedy approach.

* **Bellman-Ford Algorithm**: Computes shortest paths from a single source vertex to all other vertices in a weighted graph, handling negative weights.

### 3.1 **String Algorithms**
String algorithms focus on efficient manipulation and searching within strings.

* **Knuth-Morris-Pratt (KMP) Algorithm**: Efficient string searching algorithm that avoids re-examination of previously matched characters using a prefix table (LPS array).

* **Rabin-Karp Algorithm**: Uses hashing to find patterns in text efficiently, useful for multiple pattern matching.
...
* **Simulated Annealing**: Searches for a solution by mimicking the process of annealing in metallurgy. <AdsComponent />

## Conclusion
Understanding the various types of data structures and algorithms, including advanced graph techniques like Topological Sort and DSU, string algorithms like KMP, and problem-solving paradigms like Sliding Window and Two Pointers, is fundamental to mastering problem-solving in computer science. Knowing which to use for a particular problem is key to writing efficient and scalable code. <AdsComponent />