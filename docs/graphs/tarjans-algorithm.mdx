...
10. For node `4`, `low[4] === index[4] === 5`. Pop stack until `4`. **SCC found: {4}** (Wait, stack was `[0, 1, 2, 3, 4]`, pop `4`). Stack becomes `[0, 1, 2, 3]`.
11. For node `3`, `low[3] === index[3] === 4`. Pop stack until `3`. **SCC found: {3}**. Stack becomes `[0, 1, 2]`.
...
public class TarjanSCC {
    ...
    private void sccUtil(int u, int[] low, int[] disc, boolean[] stackMember, Stack<Integer> st, List<List<Integer>> result) {
        disc[u] = low[u] = ++time;
        st.push(u);
        stackMember[u] = true;
