...
* $E$: Number of edges
* $W$: Maximum width of the tree/graph
* $H$: Maximum height/depth of the tree/graph
...
| **Space Complexity** | $O(V)$ for graphs (or $O(W)$ for trees) — can be high for wide structures. | $O(V)$ for graphs (or $O(H)$ for trees) — memory efficient for deep/narrow structures. |
